...
app.factory('highScoreStorage', function(){ let service = {}
...
//struktur score disimpan dalam format JSON {nama,wpm}
service.addScore = function(newScore){ hs.push(newScore)
localStorage.setItem('highScores', JSON.stringify(hs))
}
service.getScore = function(){ return hs
}

return service
})

app.factory('questionStorage', function(){ let service = {}
...
let idx = Math.floor(Math.random() * q.length); let generatedTexts = q[idx].split(' ')
.map(text => {
...
app.controller('typingController',
function($scope, $rootScope, highScoreStorage,questionStorage){
$scope.texts;
$scope.playerInput = ''
$scope.index = 0;
$scope.startTime = 0;
...
finalTime += (
$scope.texts
.filter(t => t.status == 'wrong').length * 300
)
